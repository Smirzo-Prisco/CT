...
if((is_numeric($_POST['mesi']) === true) && ($_POST['mesi'] >= 0) && ($_POST['mesi'] <= 12)) {
    ...
    if(gdrcd_filter_get($_POST['conferma']) !== '1') {
        ...
        $count = gdrcd_query("SELECT COUNT(*) AS n FROM log WHERE DATE_SUB(NOW(), INTERVAL $mesi MONTH) > data_evento")['n'];
        ?>
        <div class="warning">
        ...
        gdrcd_query("DELETE FROM log WHERE DATE_SUB(NOW(), INTERVAL $mesi MONTH) > data_evento");
