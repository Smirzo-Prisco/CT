...
if($_SESSION['admin']!=1){
	...
	$result = gdrcd_query("SELECT id_araldo, nome, descrizione, tipo, proprietari FROM araldo WHERE (proprietari = -1 OR proprietari = '$id_mestiere') AND tipo!=5 ORDER BY tipo, id_araldo", 'result');
}
else if($id_gilda!=0 && $id_mestiere==0){
	$result = gdrcd_query("SELECT id_araldo, nome, descrizione, tipo, proprietari FROM araldo WHERE (proprietari = -1 OR proprietari = '$id_gilda') AND (tipo!=6 AND tipo !=12 AND tipo !=11 AND tipo !=10 AND tipo !=9 AND tipo !=8 AND tipo !=7) ORDER BY tipo, id_araldo", 'result');


}
else if($id_gilda!=0 && $id_mestiere!=0){
     if ($con_job !=0) {
	$result = gdrcd_query("SELECT id_araldo, nome, descrizione, tipo, proprietari FROM araldo WHERE (proprietari = -1) OR (proprietari = '$id_gilda' AND tipo=5) OR (proprietari = '$id_mestiere' AND tipo=6) ORDER BY tipo, id_araldo", 'result');
                 } else {
	$result = gdrcd_query("SELECT id_araldo, nome, descrizione, tipo, proprietari FROM araldo WHERE (proprietari = -1) OR (proprietari = '$id_gilda' AND tipo=5) ORDER BY tipo, id_araldo", 'result');
                 }
}
else{
	$result = gdrcd_query("SELECT id_araldo, nome, descrizione, tipo, proprietari FROM araldo WHERE (proprietari = -1) AND (tipo!=6 AND tipo!=5) ORDER BY tipo, id_araldo", 'result');
}
}
