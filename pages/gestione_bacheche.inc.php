    ...
    if($_SESSION['admin']!=1) {
        ...
        <div class="page_body">
            ...
            if(gdrcd_filter('get', $_POST['op']) == $MESSAGE['interface']['administration']['forums']['submit']['insert']) { ?>
                ...
                gdrcd_query("INSERT INTO araldo (nome, descrizione, tipo, proprietari) VALUES ('".gdrcd_filter('in', $_POST['nome'])."', '".gdrcd_filter('in', $_POST['descrizione'])."', ".gdrcd_filter('num', $_POST['tipo']).", ".gdrcd_filter('num', $_POST['owner']).")"); ?>
                <div class="warning">
            ...
            if($_POST['op'] == $MESSAGE['interface']['administration']['forums']['submit']['edit']) {
                ...
                gdrcd_query("UPDATE araldo SET nome ='".gdrcd_filter('in', $_POST['nome'])."', descrizione ='".gdrcd_filter('in', $_POST['descrizione'])."', tipo = ".gdrcd_filter('num', $_POST['tipo']).", proprietari = ".gdrcd_filter('num', $_POST['owner'])." WHERE id_araldo = ".gdrcd_filter('num', $_POST['id_record'])." LIMIT 1");
                ?>
                <div class="warning">
            ...
            if((gdrcd_filter('get', $_POST['op']) == 'edit') || (gdrcd_filter('get', $_REQUEST['op']) == 'new')) {
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_bacheche" method="post" class="form_gestione">
                        ...
                        <div class='form_label'>
                            <?php echo 'Descrizione bacheca'; ?>
                        </div>
                        <div class='form_field'>
                            <input name="descrizione" value="<?php echo $loaded_record['descrizione']; ?>" />
                        </div>
                        ...
                        <div class='form_field'>
                            <!-- Elenco dei tipi -->
                            <select name="tipo">
                                <option value="<?php echo ONGAME; ?>"
                                    <?php if($loaded_record['tipo'] == ONGAME) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][ONGAME]); ?>
                                </option>
                                <option value="<?php echo INFO; ?>"
                                    <?php if($loaded_record['tipo'] == INFO) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][INFO]); ?>
                                </option>
                                <option value="<?php echo COMUNICAZIONI; ?>"
                                    <?php if($loaded_record['tipo'] == COMUNICAZIONI) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][COMUNICAZIONI]); ?>
                                </option>
                                ...
                                <option value="<?php echo SOLOMESTIERE; ?>"
                                    <?php if($loaded_record['tipo'] == SOLOMESTIERE) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOMESTIERE]); ?>
                                </option>
                                ...
                                <option value="<?php echo SOLOGUIDES; ?>"
                                    <?php if($loaded_record['tipo'] == SOLOGUIDES) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOGUIDES]); ?>
                                </option>
                                <option value="<?php echo SOLOCAPOGILDA; ?>"
                                    <?php if($loaded_record['tipo'] == SOLOCAPOGILDA) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOCAPOGILDA]); ?>
                                </option>
                                <option value="<?php echo SOLOCAPOMESTIERI; ?>"
                                    <?php if($loaded_record['tipo'] == SOLOCAPOMESTIERI) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOCAPOMESTIERI]); ?>
                                </option>
                                <option value="<?php echo SOLOADMIN; ?>"
                                    <?php if($loaded_record['tipo'] == SOLOADMIN) {echo "selected";} ?>>
                                    <?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOADMIN]); ?>
                                </option>
                        ...
                        <div class='form_field'>
                            ...
                            $mestieri = gdrcd_query("SELECT id_mestiere, nome FROM mestiere", 'result'); 
                            ?>
                            ...
                            <select name="owner">
                                ...
                                gdrcd_query($gilde, 'free');
                                   ...
                                   while($option = gdrcd_query($mestieri, 'fetch')) { ?>
                                    <option value="<?php echo gdrcd_filter('out', $option['id_mestiere']); ?>"
                                        <?php if(($loaded_record['proprietari'] == $option['id_mestiere']) && ($loaded_record['tipo'] == SOLOMESTIERE)) {
                                            echo 'SELECTED';
                                        } ?>>
                                        <?php echo gdrcd_filter('out', $option['nome']); ?>
                                    </option>
            ...
            if((isset($_POST['op']) === false) && (isset($_REQUEST['op']) === false)) {
                ...
                //Lettura record
                $result = gdrcd_query("SELECT id_araldo, nome, descrizione, tipo FROM araldo ORDER BY nome LIMIT ".$pagebegin.", ".$pageend."", 'result');
