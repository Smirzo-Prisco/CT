<div class="pagina_link_menu">
    ...
    <div class="page_body">
        ...
        foreach($PARAMETERS[$mkey] as $key => $link_menu) {
        			##########################roba bruttissima per ct news
                    $query = "SELECT ctnews_letto FROM personaggio WHERE nome='".$_SESSION['login']."'";
                    $letto = gdrcd_query($query);
                    
                    if($letto['ctnews_letto']==0 && strpos($link_menu['image_file'], 'news')){
                    $link_menu['image_file'] = '../menu/ctnews_gif.gif';
                    $link_menu['image_file_onclick'] = '../menu/ctnews_gif.gif';
                    }
                    ##########################
            if( ! empty($link_menu['url'])) {
                ...
                if(empty($link_menu['image_file'])) {
                    ...
                    $content = '<img onclick="'.$link_menu['onclick'].'" class="menu_img" src="/themes/'.$PARAMETERS['themes']['current_theme'].'/imgs/'.$mkey.'/'.$link_menu['image_file'].'" alt="'.gdrcd_filter('out',
                                                                                                                                                              $link_menu['text']
                ...
                foreach($link_menu as $k => $v) {
                    if( ! in_array($k, ['text', 'image_file', 'url', 'image_file_onclick', 'sprite', 'onclick'])) {
                        echo $k.'="'.$v.'"';
                    }
                }
                echo $content.'</a></div>';
