...
if($PARAMETERS['mode']['check_forum'] === 'ON') {
    ...
    while($row = gdrcd_query($result, 'fetch')) {
        if(($row['tipo'] <= PERTUTTI) || (($row['tipo'] == SOLORAZZA) && ($_SESSION['id_razza'] == $row['proprietari'])) || (($row['tipo'] == SOLOGILDA) && (strpos($_SESSION['gilda'], '*'.$row['proprietari'].'*') != false)) || (($row['tipo'] == SOLOMASTERS) && ($_SESSION[' master']==1 || $_SESSION['moderatore']==1 || $_SEOSSION['admin']==1)) {
