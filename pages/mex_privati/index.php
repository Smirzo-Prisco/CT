...
<html lang="en">
    <head>
        ...
        <link rel="stylesheet" href="../../pages/mex_privati/new_sms_tablet.css">
    </head>
    <body>
        <?php
            ...
            while ($row = gdrcd_query($result_combined, 'fetch')) {
                ...
                if ($row['tipo'] == 'individuale') {
                    ...
                    if ($mittente === 'Segnalazione') $avatar_url = '../../pages/msg/img/segnalazione.png';
                    elseif ($mittente === 'Calendario') $avatar_url = '../../pages/msg/img/calendario.png';
                    else $avatar_url = $display_data['avatar_url'];
                    ...
                    if (is_unread($personaggio, $row)) {
                        if ($row['ongame'] == 1) $new_messages_on = true; // Nuovo messaggio ON individuale
                        else $new_messages_off = true; // Nuovo messaggio OFF individuale
                    }
                    ...
                    if (!isset($titoli_gruppi_memorizzati[$conversazione_id])) {
                        ...
                        if ($row_titolo) $titoli_gruppi_memorizzati[$conversazione_id] = $row_titolo['titolo_gruppo'];
                        else $titoli_gruppi_memorizzati[$conversazione_id] = "Gruppo Sconosciuto"; // Valore di fallback
                    }

                    // Usa il titolo del gruppo memorizzato
                    $display_name = $titoli_gruppi_memorizzati[$conversazione_id];
                    $avatar_url = '../../themes/crystal/imgs/sms/img_gruppo'; // Recupera l'avatar del gruppo
                    ...
                    if ($row['tipo'] == 'gruppo' && is_group_message_unread($conversazione_id, $_SESSION['login'])) {
                        if ($row['ongame'] == 1) $new_messages_on = true; // Nuovo messaggio ON di gruppo
                        else $new_messages_off = true; // Nuovo messaggio OFF di gruppo
                    }
                    ...
                    $avatar_url = '../../themes/crystal/imgs/sms/img_globale'; // Puoi impostare un avatar specifico per i messaggi globali
                ...
                if ($row['ongame'] == 1) $messaggi_on .= $html;
                else $messaggi_off .= $html;
            }
        ...
        <div class="container">
            <div class="sidebar">X
                <div class="header"><div class="header-container"><h1 class="header-title">Messaggi Privati</h1></div></div>
                <div class="toggle-buttons">
                    <div class="toggle-button" id="toggle-off"><img src="<?= $off_button_image ?>" alt="Messaggi Off"></div>
                    <div class="toggle-button active" id="toggle-on"><img src="<?= $on_button_image ?>" alt="Messaggi On"></div>
                </div>
                ...
                <!-- Pulsante per espandere la sidebar -->
                <button class="expand-sidebar-button">Chiudi</button>
            </div>
            <!-- Main Content Area -->
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // Aggiungi il pulsante per espandere la sidebar
                sidebarToggleButton.classList.add('sidebar-toggle-button');
                sidebarToggleButton.textContent = 'Chiudi';
                sidebar.appendChild(sidebarToggleButton);

                // Nasconde la sidebar e mostra la conversazione
                messageItems.forEach(item => {
                    item.addEventListener('click', () => {
                        sidebar.classList.add('reduced');  // Riduci la sidebar
                        mainContent.classList.add('active');  // Mostra la conversazione
                    });
                });

                // Espandi la sidebar quando si clicca sul pulsante "Espandi Sidebar"
                sidebarToggleButton.addEventListener('click', () => {
                    sidebar.classList.remove('reduced');  // Espandi la sidebar
                    mainContent.classList.remove('active');  // Nascondi la conversazione
                });
            });

            // Funzione per mostrare la conversazione e nascondere la sidebar
            document.addEventListener('DOMContentLoaded', () => {
                const sidebar = document.querySelector('.sidebar');
                const mainContent = document.querySelector('.main-content');
                const expandButton = document.querySelector('.expand-sidebar-button');

                // Funzione per mostrare la conversazione e nascondere la sidebar
                function mostraConversazione() {
                    sidebar.classList.add('reduced');  // Riduci la sidebar
                    mainContent.classList.add('active');  // Mostra la conversazione
                    expandButton.style.display = 'block'; // Mostra il tasto per espandere la sidebar
                }

                // Funzione per espandere di nuovo la sidebar e nascondere la conversazione
                expandButton.addEventListener('click', function() {
                    sidebar.classList.remove('reduced');  // Espandi la sidebar
                    mainContent.classList.remove('active');  // Nascondi la conversazione
                    this.style.display = 'none'; // Nasconde il pulsante di espansione
                });

                // Funzione per aprire la conversazione quando selezioni un nuovo personaggio e tipologia
                document.getElementById('new-sms-select').addEventListener('change', function () {
                    const pgSelezionato = this.value;
                    const tipologiaSelezionata = document.getElementById('ongame-offgame').value;

                    if (pgSelezionato && tipologiaSelezionata) {
                        mostraConversazione();

                        // Simula l'inizio di una conversazione, puoi modificare questa parte per inviare una richiesta AJAX al server se necessario
                        console.log("Inizio conversazione con:", pgSelezionato, "Tipologia:", tipologiaSelezionata);
                        
                        // Chiudi il popup una volta selezionato
                        document.getElementById('new-sms-popup').style.display = 'none';
                    }
                });
