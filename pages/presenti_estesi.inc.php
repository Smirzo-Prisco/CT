...
<div class="pagina_presenti_estesa">
  ...
  <thead>
    <tr>
      <th colspan="7" style="font-size: 12px; color: #8f8f8f; font-family: 'DejaVu Serif'">PRESENTI: <?php echo $presenti['numero']?></th>
    </tr>
  </thead>
  <tbody>
    ...
    $mappa_corrente = '';
            ...
            if ($record['is_invisible'] == 1)  {
                ...
                if ($record['mappa'] != $mappa_corrente)  {
                    ...
                    echo '<tr class="mappa"><td colspan="8" style="text-transform: uppercase;">'.gdrcd_filter('out', $mappa_corrente).'</span></td></tr>';
                }//if
            ...
            if (empty($luogo_corrente) === true) {
                ...
                if ($ultimo_luogo_corrente != $luogo_corrente)  {
                    ...
                    echo '<tr class="luogo"><td colspan="8" style="text-transform: uppercase;">'.gdrcd_filter('out', $luogo_corrente).'</td></tr>';
                } //if
            } else {
                if ($ultimo_luogo_corrente != $luogo_corrente)   {
                    $ultimo_luogo_corrente = $luogo_corrente;
                    if ($record['is_invisible'] == 0)  {
                        if (($PARAMETERS['mode']['mapwise_links'] == 'OFF')) { #||($record['ultima_mappa']==$_SESSION['mappa'])
                            echo '<tr class="third_header"><td colspan="8" style="text-transform: uppercase;"><a href="main.php?dir='.$record['ultimo_luogo'].'&map_id='.$record['ultima_mappa'].'">'.gdrcd_filter('out', $luogo_corrente).'</a></td></tr>';
                        } else  {
            ...
            echo '<td width="5%"><img width="50" height="50" src="'.$row['url_img_chat'].'"/></td>';
           ...
           // echo '<td><a href="main.php?page=messages_center&op=create&reply_dest='.$record['nome'].'" class="link_sheet"><img src="themes/crystal/imgs/presenti/sms_presenti.png"/></a></td>';
				echo'<td>';?>
                <a href="#" onclick="changeFrame('pages/mex_privati/multi_message.php?destinatari=<?php echo urlencode($record['nome']); ?>'); document.getElementById('id01').style.display='block';">
                <img src="themes/crystal/imgs/presenti/sms_presenti.png"/>
                </a>
				<?php
                echo '</td>';
            ...
            if ($num_incl == 1) {
                ...
                $query = "SELECT clgpersonaggioruolo.personaggio, clgpersonaggioruolo.nickname, personaggio.cognome, ruolo.immagine, ruolo.capo, ruolo.nome_ruolo FROM ruolo JOIN clgpersonaggioruolo ON clgpersonaggioruolo.id_ruolo = ruolo.id_ruolo JOIN personaggio ON personaggio.nome = clgpersonaggioruolo.personaggio WHERE personaggio.nome=".'"'.gdrcd_filter('out', $record['nome']).'"';
                $row = gdrcd_query($query);
                $alt = $row['nickname'] == '' ? $row['nome_ruolo'] : $row['nickname'];
                echo '<td><img width="25" height="25" src="imgs/guilds/'.$row['immagine'].'" title="'.gdrcd_filter('out', $alt).'"/></td>';
            } else {
            ...
            echo '<td>';
                    ...
                    if($row_pri['admin']==1){
                    echo '<img src="themes/crystal/imgs/staff/Admin.png" width="20" height="20" title="Coordinatore">';
                    }
                    if($row_pri['moderatore']==1){
                    	echo '<img src="themes/crystal/imgs/staff/Moderatore.png" width="20" height="20" title="Moderatore">';
                    }
                    if($row_pri['master']==1){
                    	echo '<img src="themes/crystal/imgs/staff/Master.png" width="20" height="20" title="Master">';
                    }
                    if($row_pri['guida']==1){
                    	echo '<img src="themes/crystal/imgs/staff/Guida.png" width="20" height="20" title="Guida">';
                    }
                    if($row_pri['grafico']==1){
                    	echo '<img src="themes/crystal/imgs/staff/Grafico.png" width="20" height="20" title="Grafico">';
                    }
