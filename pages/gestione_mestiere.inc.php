...
<!-- Titolo della pagina -->
    ...
    <?php /*modifica o elimina*/
           ...
           $capo = gdrcd_query($capo_result, 'fetch');
            ...
            if($jobs['COUNT(*)'] >= $PARAMETERS['settings']['guilds_limit']) {
                ...
                if ($gilda['mestiere'] == 1) {
                $check_fuoco = gdrcd_query("SELECT id_abilita, nome FROM clgpersonaggioabilita WHERE nome = '".$_POST['nome']."' && id_abilita = 40", 'result');
                if(gdrcd_query($check_fuoco, 'num_rows') < 1) {
                ...
                $check_bianca = gdrcd_query("SELECT id_abilita, nome FROM clgpersonaggioabilita WHERE nome = '".$_POST['nome']."' && id_abilita = 41", 'result');
                if(gdrcd_query($check_bianca, 'num_rows') < 1) {
    ...
    /*Seleziono i ruoli su cui l'account ha competenza*/
                        ...
                        //$members = "SELECT clgpersonaggiomestiere.*, ruolo_mestiere.* FROM clgpersonaggiomestiere JOIN ruolo_mestiere ON clgpersonaggiomestiere.id_ruolo=ruolo_mestiere.id_ruolo WHERE ruolo_mestiere.mestiere IN (SELECT ruolo_mestiere.mestiere FROM clgpersonaggiomestiere JOIN ruolo_mestiere ON clgpersonaggiomestiere.id_ruolo = ruolo_mestiere.id_ruolo WHERE clgpersonaggiomestiere.personaggio= '".$_SESSION['login']."' AND ruolo_mestiere.mestiere>-1) AND clgpersonaggiomestiere.conferma_mestiere != 0  ORDER BY ruolo_mestiere.id_ruolo, clgpersonaggiomestiere.personaggio ASC";
                        $members = "SELECT clgpersonaggiomestiere.*, ruolo_mestiere.* FROM clgpersonaggiomestiere JOIN ruolo_mestiere ON clgpersonaggiomestiere.id_ruolo=ruolo_mestiere.id_ruolo WHERE clgpersonaggiomestiere.conferma_mestiere != 0  ORDER BY ruolo_mestiere.id_ruolo, clgpersonaggiomestiere.personaggio ASC";
    ...
    <form action="main.php?page=gestione_mestiere" method="post">
    <tr>
    <td>
    <input name="nome" type=hidden value="<?php echo $row['personaggio']; ?>">
    <img src="imgs/mestieri/<? echo $row['immagine']; ?>" border=0 title="<? echo $row['nome_ruolo']; ?>">
    ...
    //seleziono la gerarchia del mestiere
    //$query = "SELECT ruolo_mestiere.id_ruolo, ruolo_mestiere.nome_ruolo, mestiere.nome FROM ruolo_mestiere JOIN mestiere ON ruolo_mestiere.mestiere = mestiere.id_mestiere WHERE ruolo_mestiere.mestiere IN (SELECT ruolo_mestiere.mestiere FROM clgpersonaggiomestiere JOIN ruolo_mestiere ON clgpersonaggiomestiere.id_ruolo = ruolo_mestiere.id_ruolo WHERE clgpersonaggiomestiere.personaggio= '".$_SESSION['login']."' AND ruolo_mestiere.mestiere>-1) ORDER BY mestiere.nome, ruolo_mestiere.id_ruolo ASC, ruolo_mestiere.nome_ruolo";
    $query = "SELECT ruolo_mestiere.id_ruolo, ruolo_mestiere.nome_ruolo, mestiere.nome 
    FROM ruolo_mestiere JOIN mestiere ON ruolo_mestiere.mestiere = mestiere.id_mestiere 
    ORDER BY mestiere.nome, ruolo_mestiere.id_ruolo ASC, ruolo_mestiere.nome_ruolo";
    $result = gdrcd_query($query, 'result');
