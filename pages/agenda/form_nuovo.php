...
if (isset($_POST['submit']) && $_POST['submit']=="invia")
{
  ...
  if($result = mysql_query($sql) or die (mysql_error()))
  {
    ...
    if ($destinatario == 'Muryu Tamashi') {
    $query = "SELECT * FROM Personaggio WHERE IDGilda > 2299 AND IDGilda < 2400";
    $resultq = mysql_query($query) or die (mysql_error());
      if(mysql_num_rows($resultq) > 0)
      {
        while($fetchq = mysql_fetch_array($resultq))
        {
        $nome = stripslashes($fetchq['Nome']);
...
while ($rs = mysql_fetch_array($Result)) {
	echo '<option value='.$rs['Nome'].'>'.htmlspecialchars($rs['Nome']).'</option>';
}
$rs->close;
?>
...
while ($rs = mysql_fetch_array($Result)) {
	echo '<option value='.$rs['NomeCorporazione'].'>'.htmlspecialchars($rs['NomeCorporazione']).'</option>';
}
$rs->close;
?>
</select>;
...
while ($rs = mysql_fetch_array($Result)) {
	echo '<option>'.htmlspecialchars($rs['Breve']).'</option>';
}
$rs->close;
?>
</select><br><br>
