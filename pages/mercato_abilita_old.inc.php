...
if(!isset($_REQUEST['pg']))
{
    ...
    if((gdrcd_filter('get', $_REQUEST['op']) == 'addskill') && (($_SESSION['login'] == gdrcd_filter('out', $_REQUEST['pg'])) || ($_SESSION['admin']==1 || $_SESSION['moderatore']==1))) {
        ...
        $esperienza_rimanente = $esperienza_rimanente['esperienza_r'];
        ...
        elseif(((int)$esperienza_rimanente / 5) >= 1 && $livello['grado'] < $abilita['max_lvl']) {
...
?>
    <div class="form_info">
        <?php 
        $esperienza_rimanente = gdrcd_query("SELECT esperienza_r FROM personaggio WHERE nome='".gdrcd_filter('in', $_SESSION['login'])."'");
        $esperienza_rimanente = floor((int)$esperienza_rimanente['esperienza_r'] / 5);
