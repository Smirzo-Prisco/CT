                    ...
                    <tr>
                        ...
                        <td>
                            <div class="abilita_scheda_sub">
                                ...
                                if((((($ranks[$row['id_abilita']] + 1) * $PARAMETERS['settings']['px_x_rank']) <= ($px_totali_pg - $px_spesi)) && (gdrcd_filter('get', $_REQUEST['pg']) == $_SESSION['login']) && ($ranks[$row['id_abilita']] < $PARAMETERS['settings']['skills_cap'])) || ($_SESSION['admin']==1 || $_SESSION['moderatore']==1)) { ?>
                                    [<a href="main.php?page=scheda&pg=<?php echo gdrcd_filter('url', $_REQUEST['pg']
                                    ) ?>&op=addskill&what=<?php echo $row['id_abilita'] ?>">+</a>]
                                    <?php if(($_SESSION['admin']==1 || $_SESSION['moderatore']==1) && ($ranks[$row['id_abilita']] > 0)) { ?>
...
<div class="elenco_abilita"><!-- Elenco abilità -->
    ...
    //carico l'elenco delle abilità
    $result = gdrcd_query("SELECT nome, car, id_abilita FROM abilita WHERE id_razza=-1 AND id_gilda = 0 OR id_razza= ".$personaggio['id_razza']." ORDER BY id_razza DESC, nome", 'result');
    $count = 0;
    $total = 0;
    ?>
    <div class="form_info">
