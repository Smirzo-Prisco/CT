...
<span style="font-size: 13px; color: #9a6353 ; font-family: DejaVu Serif; filter: drop-shadow(0 0 5px rgba(0,0,0,0.57));">CONTATTA LA MODERAZIONE</span><br>
<span style="font-size: 13px; color: #8f8f8f ; font-family: DejaVu Serif; filter: drop-shadow(0 0 5px rgba(0,0,0,0.57));">(<i>Qualora il richiedente scegliesse di restare anonimo, il suo nome viene criptato dal sistema. Gestione e moderazione <u>non</u> possono vedere il suo nome.</i>)</span>

<form action="<?php echo $_SERVER['SCRIPT_NAME'] . '?' . $_SERVER['QUERY_STRING']; ?>"
                          method="post">
<span style="font-size: 13px; color: #8f8f8f ; font-family: DejaVu Serif; filter: drop-shadow(0 0 5px rgba(0,0,0,0.57));">                          
<br>                          <div class='form_label'>
                            Titolo
                        </div>
...
<?php
    ...
    ?>
    <a href="#dialog<?php echo $fetch["id"]; ?>" name="modal"><?= $titolo ?></a>
    <?php } else { ?>
    <?= $titolo ?>
    <?php } ?>
    </td>
    ...
    <div id="box"> 
		<div id="dialog<?php echo $fetch["id"]; ?>" class="window"> 
            ...
            <table style="width: 250px;" class="customTable">
                        ...
                        <form action="<?php echo $_SERVER['SCRIPT_NAME'] . '?' . $_SERVER['QUERY_STRING']; ?>"
                            ...
                            <div class="form_submit">
                                ...
                                <textarea name="testo" style="display:none;"><?php echo gdrcd_filter('in', $_POST['testo']) ?>
                                       </textarea>
                        ...
                        <form action="javascript:history.go(-1)"
                            ...
                            <div class="form_submit">
                                ...
                                <input type="hidden" name="testo"
                                       value="<?php echo gdrcd_filter('in', $_POST['testo']) ?>"/>
                                <input type="hidden" name="anonimo"
                                       value="<?php echo gdrcd_filter('out', $_POST['anonimo']) ?>"/>
...
/***** FINE Fase 2 *****/
        ...
        gdrcd_query("INSERT INTO messaggio_moderazione (titolo, messaggio, autore, data_messaggio, anonimo) VALUES ('".gdrcd_filter('in', $_POST['titolo'])."', '".gdrcd_filter('in', $_POST['testo'])."', '" . $_SESSION['login'] . "', NOW(), '" . gdrcd_filter('out', $_POST['anonimo']) . "')");
        gdrcd_query("INSERT INTO messaggioaraldo (id_messaggio_padre, id_araldo, titolo, messaggio, autore, data_messaggio, data_ultimo_messaggio, giornalista, anonimo) VALUES ('-1', '86', '" . gdrcd_filter('in', $_POST['titolo']) . "', '" . gdrcd_filter('in', $_POST['testo']) . "', '" . $_SESSION['login'] . "', NOW(), NOW(), 'no', '" . gdrcd_filter('out', $_POST['anonimo']) . "')");
