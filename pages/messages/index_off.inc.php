...
if($_GET['op'] == 'inviati') {
    $result = gdrcd_query("SELECT * FROM messaggi WHERE mittente = '".$_SESSION['login']."' AND mittente_del = 0 AND tipo = 'off' ORDER BY spedito DESC LIMIT ".$pagebegin.", ".$pageend."", 'result');
    ...
    $result = gdrcd_query("SELECT * FROM messaggi WHERE destinatario = '".$_SESSION['login']."' AND tipo = 'off' AND destinatario_del = 0 ".$extracond." ORDER BY spedito DESC LIMIT ".$pagebegin.", ".$pageend."", 'result');
