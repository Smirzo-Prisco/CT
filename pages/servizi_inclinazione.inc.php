...
<div class="pagina_servizi_gilde">
    <!-- Titolo della pagina -->
    <div class="page_title"><h2><?php echo "Inclinazioni"; ?></h2></div>
    <!-- Box principale -->
    <div class="page_body">
        ...
        <table class="customTable">
            ...
            <tr class="second_header">
                <td width="50%"><?php echo gdrcd_filter('out', $PARAMETERS['names']['corrente_name']['sing']); ?></td>
                <td width="40%"><?php echo 'Referente'; ?></td>
                <td><?php echo gdrcd_filter('out', $PARAMETERS['names']['guild_name']['members']); ?></td>
            </tr>
    <?php
        $last_type = $row['tipo'];
        ...
        while($row = gdrcd_query($result, 'fetch')) {
            ...
            ?>
            <tr>
                <td width="50%"><a href="main.php?page=servizi_inclinazione&id_inclinazione=<?php echo $row['id_inclinazione']; ?>"><?php echo gdrcd_filter('out', $row['nome']); ?></a></td>
                <td width="40%"><a href="main.php?page=scheda&pg=<?=$row['referente']?>"><?php echo gdrcd_filter('out', $row['referente']); ?></a></td>
...
<?php /* Visualizzazione estesa gilda */
            ...
            $result = gdrcd_query($query, 'result'); ?>
                    <table class="customTable">
                        ...
                        <tr class="second_header">
                            <td><div>&nbsp;</div></td>
                            <td><div><?php echo gdrcd_filter('out', $MESSAGE['interface']['guilds']['member']); ?></div></td>
                            <!-- Elenco -->
                            <?php
                            while($row = gdrcd_query($result, 'fetch')){
                            ?>
                        <tr>
                            <td><div><img src="imgs/inclinazioni/<?php echo gdrcd_filter('out', $row['immagine']); ?>"></div></td>
                            <td>
                                <div>                     
