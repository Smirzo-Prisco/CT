...
/*************** PERMESSI DI ACCESSO ***************/ 
if ($_SESSION['admin'] != 1 && $_SESSION['capomestiere'] != 1 && $_SESSION['master'] != 1 && $_SESSION['moderatore'] != 1) {
...
<?php /*Form di scelta del log (visualizzazione di base)*/
    ...
    { ?>
        ...
        <div class="panels_box">
            ...
            <?php //log chat solo ad admin, mod e master
...
if (isset($_REQUEST['op']) == 'view_date') {
    // Formattazione date
    $date_b = sprintf('%04d-%02d-%02d %02d:%02d:00',
    ...
    $date_e = sprintf('%04d-%02d-%02d %02d:%02d:00',
    ...
    $chat_id = gdrcd_filter('num', $_POST['luogo']);

    // Controllo autorizzazioni
    $is_authorized = gdrcd_query("SELECT COUNT(*) AS count
                      FROM autorizzazioni_log
                      WHERE user_name = '" . gdrcd_filter('out', $_SESSION['login']) . "'
                        AND chat_id = {$chat_id}
                        AND start_time <= '{$date_b}'
                        AND end_time >= '{$date_e}'", 'result');

    $authorization = gdrcd_query($is_authorized, 'fetch')['count'] > 0;

        
    //admin leggono tutto
    if ($_SESSION['admin'] == 1) {
        $result = gdrcd_query("SELECT * FROM chat WHERE stanza = {$chat_id} AND ora BETWEEN '{$date_b}' AND '{$date_e}' ORDER BY ora ASC", 'result');
    } else if ($authorization) {
        // Master autorizzati vedono tutto
        $result = gdrcd_query("SELECT * FROM chat WHERE tipo != 'S' AND backing = '0' AND stanza = {$chat_id} AND ora BETWEEN '{$date_b}' AND '{$date_e}' ORDER BY ora ASC", 'result');
    } else {
        // Trova il primo e l'ultimo messaggio di tipo M scritto dall'utente
        $quest_period = gdrcd_query("SELECT MIN(ora) AS start_time, MAX(ora) AS end_time
                        FROM chat
                        WHERE stanza = {$chat_id}
                        AND tipo = 'M'
                        AND CHAR_LENGTH(testo) >= 1500
                        AND mittente = '" . gdrcd_filter('out', $_SESSION['login']) . "'
                        AND ora BETWEEN '{$date_b}' AND '{$date_e}'", 'result');

        $period = gdrcd_query($quest_period, 'fetch');

        // Se ci sono periodi validi, usa i limiti temporali per filtrare i log
        if ($period['start_time'] && $period['end_time']) {
            $start_time = $period['start_time'];
            $end_time = $period['end_time'];

            // Recupera tutti i messaggi tra il primo e l'ultimo messaggio di tipo M
            $result = gdrcd_query("SELECT * 
                        FROM chat 
                        WHERE stanza = {$chat_id} 
                            AND ora BETWEEN '{$start_time}' AND '{$end_time}' 
                            AND tipo != 'S' 
                            AND backing = '0' 
