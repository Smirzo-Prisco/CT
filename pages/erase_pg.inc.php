...
// richiamo tutti pg
$tutti_pg = gdrcd_query("SELECT * FROM personaggio WHERE DATE_ADD(ora_entrata, INTERVAL 150 DAY) <= NOW() AND (esilio < NOW() || esilio IS NULL) ORDER BY nome", 'result');
?>

<form action="main.php?page=erase_pg" method="post" name="cancellaselezione">
    <table class="customTable">
        <tr><td colspan="2">Personaggi inattivi da 5 mesi</td></tr>
        <tr class="second_header"><td>Nome</td><td></td></tr>
        <?php while ($row = mysqli_fetch_array($tutti_pg)){ ?>
        <tr>
            <td><a href="main.php?page=scheda&pg=<?php echo $row['nome']; ?>"><?php echo $row['nome']; ?></a></td>
