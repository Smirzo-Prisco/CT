        ...
        <div class="panels_box">
            ...
            <div class="elenco_record_gioco">
                <table class="customTable">
                    ...
                    <?php //Lettura record
                $result = gdrcd_query("SELECT * FROM PuntiMestiere LEFT JOIN messaggioaraldo ON messaggioaraldo.id_messaggio = PuntiMestiere.id_messaggio WHERE nome = '" . gdrcd_filter('in', 
                          $_REQUEST['pg']) . "' ORDER BY data_evento DESC LIMIT ".$pagebegin.", ".$pageend."", 'result');
                ...
                while ($record = gdrcd_query($result, 'fetch'))
                    { 
                     ...
                     $record['titolo'] = "Ruolata di mestiere";
                     }
                     ?>
                ...
                <div class="pager">
                    <?php if ($totaleresults > $PARAMETERS['settings']['view_logs'])
                    {
                        ...
                        for ($i = 0; $i <= floor($totaleresults / $PARAMETERS['settings']['view_logs']); $i++)
                        {
                            ...
                            {
                                ?>
                                <a href="main.php?page=scheda_px_mestiere&pg=<?php echo $_REQUEST['pg']; ?>&offset=<?php echo $i; ?>"><?php echo $i + 1; ?></a>
                            <?php } else
                            {
                                echo ' ' . ($i + 1) . ' ';
                            }
