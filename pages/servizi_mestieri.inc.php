...
<div class="pagina_servizi_gilde">
    ...
    <div class="page_title">
        <h2><?=gdrcd_filter('out', $PARAMETERS['names']['job_name']['plur'])?></h2>
    </div>
    <!-- Box principale -->
    <div class="page_body">
        ...
        if(isset($_REQUEST['id_mestiere']) === false) {
            ...
            <table class="customTable">
                <tr class="second_header"><td>Lavoro Libero</td><td><?=gdrcd_filter('out', $PARAMETERS['names']['job_name']['members'])?></td></tr>
<?php
...
?>
            ...
            $result = gdrcd_query($query, 'result')?>
                    ...
                    <table class="customTable">
                        <tr class="second_header"><td><div>&nbsp;</div></td><td><div></div></td></tr>
                        <!-- Elenco -->
                        <?php
                        while($row = gdrcd_query($result, 'fetch')) { ?>
                        <tr>
                            <td><div><img src="imgs/mestieri/<?=gdrcd_filter('out', $row['immagine'])?>" /></div></td>
