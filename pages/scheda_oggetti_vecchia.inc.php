<div class="pagina_scheda_oggetti">
    ...
    <div class="page_body">
        <div class="panels_box">
            ...
            $query = "SELECT oggetto.id_oggetto, oggetto.tipo, oggetto.nome AS nome_oggetto, oggetto.descrizione, oggetto.urlimg, oggetto.ubicabile, oggetto.difesa, oggetto.attacco, oggetto.bonus_car0, oggetto.bonus_car1, oggetto.bonus_car2, oggetto.bonus_car3, oggetto.bonus_car4, oggetto.bonus_car5, clgpersonaggiooggetto.* FROM clgpersonaggiooggetto LEFT JOIN oggetto ON clgpersonaggiooggetto.id_oggetto=oggetto.id_oggetto WHERE clgpersonaggiooggetto.nome = '" . gdrcd_filter('in',
                        ...
                        <tr>
                            ...
                            <td class="casella_elemento">
                                ...
                                <div class="inventario_img">
                                    <img
                                        src="/imgs/items/<?php echo gdrcd_filter('out',
                                            $record['urlimg']); ?>"/>
                                </div>
                            ...
                            <td class="casella_elemento">
                                ...
                                echo '<hr>';
                                echo gdrcd_filter('out',
                                    $MESSAGE['interface']['sheet']['items']['list']['notes'] . ": " . $record['commento_master']); ?>
                                    <?php } else
                                {
                                    if ($_SESSION['login'] != gdrcd_filter('get', $_REQUEST['pg']))
                                    {//if
                                ...
                                if($tipo['descrizione']=='Pandora'){
                                $gilda = gdrcd_query("SELECT mestiere.nome FROM mestiere JOIN personaggio ON personaggio.id_mestiere=mestiere.id_mestiere WHERE personaggio.nome = '".$_SESSION['login']."'");
                                if(($record['commento_shop'] != '') && (($_SESSION['login'] != gdrcd_filter('get',
                                            $_REQUEST['pg']) && ($_SESSION['admin']!=1 && $gilda!='Secret Pandora' )) || ($_SESSION['login'] == gdrcd_filter('get',$_REQUEST['pg']))))
                                {
                                echo '<hr>';
                                echo gdrcd_filter('out',
                                    $MESSAGE['interface']['sheet']['items']['list']['notes'] . ": " . $record['commento_shop']); ?>
                                    <?php } elseif($record['commento_shop']!='')
                                {
                                    if ($_SESSION['login'] != gdrcd_filter('get', $_REQUEST['pg']))
                                    {//if
                                        ?>
								...
								$gilda = gdrcd_query("SELECT mestiere.nome FROM mestiere JOIN personaggio ON personaggio.id_mestiere=mestiere.id_mestiere WHERE personaggio.nome = '".$_SESSION['login']."'");
                                if(($record['commento_shop'] != '') && (($_SESSION['login'] != gdrcd_filter('get',
                                            $_REQUEST['pg']) && ($_SESSION['admin']!=1 && $gilda!='Shirokuro Magic Shop')) || ($_SESSION['login'] == gdrcd_filter('get',$_REQUEST['pg']))))
                                {
                                echo '<hr>';
                                echo gdrcd_filter('out',
                                    $MESSAGE['interface']['sheet']['items']['list']['notes'] . ": " . $record['commento_shop']); ?>
