...
<?php if ($tipo_oggetto_selezionato === null) { ?>
    ...
    <form class="form_gestione" action="main.php?page=oggetto_aggiungi" method="post">
...
if ($_SESSION['admin'] != 1 && $_SESSION['master'] != 1) {
    ...
    switch ($tipo_oggetto_selezionato) {
        case 'standard':
            $categorie_escluse = ['Arma di Gilda', 'Armi', 'Droga', 'Magic Shop', 'Medicine', 'Mods', 'Secret Pandora'];
            break;
        case 'arma':
            $categorie_permesse = ['Arma di Gilda', 'Armi', 'Secret Pandora', 'Magic Shop'];
            break;
        case 'curativo':
            $categorie_permesse = ['Magic Shop', 'Medicine', 'Secret Pandora'];
            break;
        case 'statistica':
