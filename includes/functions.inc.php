...
function gdrcd_bbcoder($str) {
    ...
    $replace = [
        '<br />',
        '<br />',
        '<details class="spoiler"><summary class="spoiler-summary">ESPANDI CONTENUTO (?)</summary><br>$1</details>',
        '<details class="spoiler"><summary class="spoiler-summary">$1</summary><br>',
        '$1</details>',
        ...
        '</blockquote></div>'*/
        '<table class="quote-table" align="center"><tr><td align="justify"><legend class="quote-legend">'.$MESSAGE['interface']['forums']['link']['quote'].'</legend>',
        '<table class="quote-table" align="center"><tr><td align="justify"><legend class="quote-legend">$1 ha scritto</legend>',
        '</td></tr></table>',
        '<textarea disabled="true" style="border: none;background-color:none; resize: none;">$1</textarea>'
         ];
 ...
 * @Param: 
 * destinatario: nome pg destinatario
 * msg: messaggio da inviare
 *
 * RETURN: bool
 */
function gdrcd_sendSystemMsg($destinatario, $msg) {
    $retmsg=false;
		
    // recupero gruppo di sistema
	$systemGroup = gdrcd_query("SELECT msggrpuser.idgroup FROM msggrpuser LEFT JOIN msggrp on msggrp.idgroup=msggrpuser.idgroup AND msggrpuser.nome='" . $destinatario . "' WHERE msggrp.ctgroup='SYSTEM'");
    $idSystem = gdrcd_filter('num', $systemGroup['idgroup']);
	
    if($idSystem <=0){
			// inserimento gruppo di sistema
			$query="INSERT INTO msggrp (dsgroup, tpgroup, ctgroup, flreadonly) VALUES ('Messaggi di sistema', 'OFF', 'SYSTEM', 'S');";
			$retQuery= gdrcd_query($query); 
			$idSystem = gdrcd_query($retQuery, 'last_id'); // recupero ultimo id inserito
			// inserimento membri
			$query="INSERT INTO msggrpuser (idgroup, nome) VALUES (".$idSystem.", 'SYSTEM');";
			gdrcd_query($query);
			$query="INSERT INTO msggrpuser (idgroup, nome) VALUES (".$idSystem.", '". $destinatario ."');";
			gdrcd_query($query);
    }
