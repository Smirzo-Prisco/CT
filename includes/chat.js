...
async function sendChatMessage() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Azione registrata con successo dal server: le bozze salvate da
            // ChatShell.jsx (recupero testo/tag persi per refresh accidentale)
            // non servono piu' per questa stanza.
            localStorage.removeItem('chat_draft_' + (window.CT_USER?.luogo ?? ''));
            localStorage.removeItem('chat_tag_draft_' + (window.CT_USER?.luogo ?? ''));
