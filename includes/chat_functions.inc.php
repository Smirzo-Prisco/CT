...
function checkSkillEffect($pg, $location) {
    ...
    // Volutamente non filtrato per id_role: l'effetto deve proseguire anche cambiando
    // giocata/quest, finché non rientra nelle condizioni di cessazione sotto.
    ...
    if ($checkPg && gdrcd_query($checkPg, 'num_rows') > 0) {
        ...
        // L'effetto cessa se il pg esce dalla fascia 20-60 (la stessa che lo fa scattare,
        // vedi registraDurata()): sotto i 20 la ferita è già troppo grave per subire
        // ulteriori effetti, sopra i 60 si è ripreso abbastanza da non risentirne più
        // (es. curato nel frattempo) — altrimenti continuo a scalare i punti.
        $salute = gdrcd_query("SELECT $type FROM personaggio WHERE nome = '$pg'")[$type];
        if ($salute < 20 || $salute > 60) {
            gdrcd_query("DELETE FROM role_durations WHERE pg_name = '$pg'");
            $motivo = $salute < 20 ? "sceso sotto i 20 punti $label" : "risalito sopra i 60 punti $label";
            chatInsertMessage($location, 'System', NULL, "Essendo $motivo, $pg smette di subire gli effetti della skill in corso", 'N');
